...
async def booking_history(db: Connection = Depends(get_db), student: dict = Depends(get_current_student)):
    ...
    rows = await db.fetch(
        ...
        SELECT
            ...
            COALESCE(
                json_agg(
                    json_build_object('id', mi.id, 'name', mi.name)
                    ORDER BY mi.name
                ) FILTER (WHERE mi.id IS NOT NULL),
                '[]'::json
            ) AS items
        FROM bookings b
        JOIN meal_slots ms ON ms.id = b.meal_slot_id
        JOIN booking_status bs ON bs.id = b.status_id
        LEFT JOIN booking_items bi ON bi.booking_id = b.id
        LEFT JOIN menu_items mi ON mi.id = bi.menu_item_id
        WHERE b.student_id = $1
        GROUP BY b.id, ms.name, bs.status_name
