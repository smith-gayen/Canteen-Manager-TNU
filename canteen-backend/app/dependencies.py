...
async def get_current_user(token: str = Depends(oauth2_scheme), db: Connection = Depends(get_db)):
    ...
    if session is None:
        raise HTTPException(status_code=401, detail="Session is expired or revoked", headers={"WWW-Authenticate": "Bearer"})

    await db.execute("UPDATE sessions SET last_used_at = CURRENT_TIMESTAMP WHERE id = $1", session["id"])
